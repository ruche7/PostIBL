...
/// @brief オブジェクトの頂点シェーダ出力構造体。
///
/// セルフシャドウの有無に関わらず出力構造体を共通にするための定義。
...
// 構造体定義の上書き用
#define VS_OUTPUT ObjectVSOutput
#define BufferShadow_OUTPUT ObjectVSOutput
...
// 頂点シェーダ
#if 1
EdgeShadowVSOutput Edge_VS(float4 Pos : POSITION, uniform int face) : POSITION 
#else
...
#endif
{
    ...
    Out.pos = CalcEnvMapFacePosition(mul(Pos, WorldViewProjMatrix), face);
...
// 頂点シェーダ
#if 1
EdgeShadowVSOutput Shadow_VS(float4 Pos : POSITION, uniform int face)
#else
...
// 頂点シェーダ
#if 1
VS_OUTPUT Basic_VS(float4 Pos, float3 Normal, float2 Tex, float2 Tex2, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon, uniform int face)
#else
...
// ピクセルシェーダ
#if 1
float4 Basic_PS(VS_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon, uniform int face)
#else
...
// 頂点シェーダ
#if 1
BufferShadow_OUTPUT BufferShadow_VS(float4 Pos, float3 Normal, float2 Tex, float2 Tex2, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon, uniform int face)
#else
...
// ピクセルシェーダ
#if 1
float4 BufferShadow_PS(BufferShadow_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon, uniform int face)
#else
    ...
    uniform int face)
{
    if (useSelfShadow)
    {
        return BufferShadow_VS(Pos, Normal, Tex, Tex2, useTexture, useSphereMap, useToon, face);
    }
    else
    {
        return Basic_VS(Pos, Normal, Tex, Tex2, useTexture, useSphereMap, useToon, face);
    }
}
...
float4 Object_PS(
    ...
    uniform bool useToon,
    uniform bool useSelfShadow,
    uniform int face) : COLOR0
{
    if (useSelfShadow)
    {
        return BufferShadow_PS(IN, useTexture, useSphereMap, useToon, face);
    }
    else
    {
        return Basic_PS(IN, useTexture, useSphereMap, useToon, face);
    }
}
